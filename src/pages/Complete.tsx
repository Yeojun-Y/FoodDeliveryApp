...
function Complete() {
  ...
  const onResponse = useCallback(async (response: any) => {
    ...
    setPreview({uri: `data:${response.mime};base64,${response.data}`}); //mime는type(jpg,png---)
  ...
  const onTakePhoto = useCallback(() => {
    return ImagePicker.openCamera({
      ...
      includeExif: true, //사진찍은휴대폰의방향을알기위해
      // cropping: true,
      saveToPhotos: true,
    })
      .then(onResponse)
  ...
  const onComplete = useCallback(async () => {
    ...
    try {
      ...
      const errorResponse = (error as AxiosError<{message: any}>).response;
      if (errorResponse) {
